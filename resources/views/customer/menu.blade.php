...
<head>
    ...
    <style>
        ...
        .btn-primary { background: #4c7c5f; color: white; width: 100%; padding: 14px; border: none; border-radius: 8px; font-weight: bold; font-size: 15px; cursor: pointer; margin-top: 10px; }
        ...
        .menu-img { width: 80px; height: 80px; background: #e2e8e4; border-radius: 8px; display: flex; align-items: center; justify-content: center; font-size: 24px; color: #4c7c5f; opacity: 0.5; }
        ...
        .add-btn { background: #f4f3ed; color: #4c7c5f; border: none; padding: 8px 15px; border-radius: 20px; font-weight: bold; cursor: pointer; align-self: flex-start; margin-top: 10px; font-size: 12px; }
...
<body>
    ...
    <div class="modal-overlay" id="welcome-modal">
        <div class="modal-box">
            <h2 style="color: #4c7c5f; margin: 0 0 5px 0;">☕ Halo!</h2>
            <p style="color: #666; font-size: 13px; margin-bottom: 25px;">Mohon isi data meja sebelum memesan</p>
            
            <input type="text" id="input-nama" class="input-field" placeholder="Nama Pemesan (mis: Rara)" required>
            <input type="number" id="input-meja" class="input-field" placeholder="Nomor Meja (mis: 4)" required>
            
            <select id="input-tipe" class="input-field" style="font-weight: bold; color: #4c7c5f;">
            ...
            <button class="btn-primary" onclick="mulaiPesan()">Lihat Menu</button>
        </div>
    </div>
    ...
    <div class="modal-overlay" id="qris-modal" style="display: none;">
        <div class="modal-box">
            ...
            <p style="color: #666; font-size: 12px; margin-bottom: 20px;">Silakan scan QRIS di bawah ini untuk membayar pesanan Anda.</p>
            
            <div style="background: #eee; width: 200px; height: 200px; margin: 0 auto 20px auto; display: flex; align-items: center; justify-content: center; border-radius: 12px;">
                <span style="font-size: 50px;">📱QRIS</span>
            </div>
            
            <button class="btn-primary" id="btn-konfirmasi-bayar" onclick="kirimPesananKeDapur()">Selesai Bayar & Pesan</button>
            <button style="background: transparent; border: none; color: #888; font-size: 12px; margin-top: 15px; cursor: pointer;" onclick="batalQris()">Batal</button>
        </div>
    </div>
    ...
    <div class="app-container">
        <div class="header">
            ...
            <p style="margin: 5px 0 0 0; font-size: 12px; opacity: 0.8;" id="header-user">Selamat datang!</p>
        </div>
        ...
        <div class="menu-list" id="menu-list">
            <div style="text-align:center; padding: 50px; color:#888;">⏳ Memuat menu...</div>
        </div>
        ...
        <div class="floating-cart">
            <div class="cart-box" id="cart-box">
                ...
                <button class="checkout-btn" onclick="tampilkanQris()">Bayar (QRIS)</button>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        // Data Session Pelanggan
        let customerData = { nama: '', meja: '' };

        const formatRupiah = (angka) => new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR', minimumFractionDigits: 0 }).format(angka);

        // 1. Fungsi Mulai Pesan (Tutup Modal Welcome)
        function mulaiPesan() {
            const nama = document.getElementById('input-nama').value.trim();
            const meja = document.getElementById('input-meja').value.trim();

            if(!nama || !meja) {
                alert("Kak, Nama dan Nomor Meja harus diisi ya!");
                return;
            }

            customerData.nama = nama;
            customerData.meja = meja;
            customerData.tipe = document.getElementById('input-tipe').value; // Tangkap tipenya

            document.getElementById('header-user').innerText = `Meja ${meja} • Kak ${nama}`;
            document.getElementById('welcome-modal').style.display = 'none';
        }

        // 2. Load Menu (Sama seperti sebelumnya)
        document.addEventListener('DOMContentLoaded', async () => {
            try {
                const res = await fetch(`${API_URL}/karyawan/pos/menus`, { headers: { 'Accept': 'application/json' } });
                const result = await res.json();
                if(res.ok) { menus = result.data; renderMenus(); }
            } catch (error) {
                document.getElementById('menu-list').innerHTML = '<div style="color:red; text-align:center;">❌ Gagal memuat menu.</div>';
            }
        });
                ...
                list.innerHTML += `
                    <div class="menu-item">
                        ...
                        <div class="menu-info">
                            ...
                            <button class="add-btn" onclick="tambahKeKeranjang(${id})">+ Tambah</button>
                        </div>
                    </div>
                `;
            });
        ...
        function updateKeranjangUI() {
            ...
            if(cart.length === 0) { cartBox.classList.remove('show'); return; }

            let totalQty = 0; subtotal = 0;
            cart.forEach(item => { totalQty += item.quantity; subtotal += item.subtotal; });

            grandTotal = subtotal + (subtotal * 0.10); // Tambah Pajak 10%
        ...
        // 5. Tembak ke Dapur setelah "Bayar"
        async function kirimPesananKeDapur() {
            ...
            const payload = {
                ...
                table_number: customerData.meja,
                order_type: customerData.tipe, // Kirim ke Backend
                payment_method: 'QRIS',
