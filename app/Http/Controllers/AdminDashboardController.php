...
class AdminDashboardController extends Controller
{
    ...
    {
        // 1. Hitung Total Penjualan (Hanya Completed) & Transaksi
        $totalPenjualan = Order::where('status', 'Completed')->sum('total_price');
        $totalTransaksi = Order::count();

        // 2. Cek Stok Kritis (Batas kritis disesuaikan menjadi 500 gram/ml)
        $stokKritis = Inventory::where('quantity', '<=', 500)->count();
